...
export default async function Home() {
  ...
  const technologyGroups = Object.entries(technologyCategoryLabels).map(([category, label]) => [label, technologies.filter((technology) => technology.category === category).map((technology) => technology.name)] as [string, string[]]).filter(([, items]) => items.length);
